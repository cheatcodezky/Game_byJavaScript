...
var winBoard = null;
var distance ;
var PuzzleLayer3 = cc.Layer.extend({
    ctor:function(){
        this._super();
        size = cc.winSize;
        over = false;
        ...
        puzzleGroup[0] = new cc.Sprite(res.tAnswer);
        puzzleGroup[0].attr({
            x:size.width/2,
            y:size.height/2,
            scale:0.5
        ...
        for(var i = 1 ; i <= number;i++){
            puzzleGroup[i] = new cc.Sprite("res/third/t"+i+".png");
            puzzleGroup[i].attr({
    ...
    removeItems:function() {
       ...
       if(puzzleGroup[6].x > puzzleGroup[1].x && puzzleGroup[6].y > puzzleGroup[4].y && (puzzleGroup[6].x - puzzleGroup[1].x)<distance && ( puzzleGroup[6].y - puzzleGroup[4].y)<distance ){
            if(puzzleGroup[1].y > puzzleGroup[3].y && puzzleGroup[2].x > puzzleGroup[5].x && (puzzleGroup[1].y - puzzleGroup[3].y)<distance &&(puzzleGroup[2].x - puzzleGroup[5].x)<distance){
                if(puzzleGroup[2].y > puzzleGroup[3].y  &&(puzzleGroup[2].y - puzzleGroup[3].y)<distance   ){
                    if(puzzleGroup[4].x >puzzleGroup[3].x && (puzzleGroup[4].x - puzzleGroup[3].x)<distance){
                        if (!over){
                            console.log("win!!!!!!!!!!!!");
                            for(var i =1 ; i<=number;i++){
...
var PuzzleScene3 = cc.Scene.extend({
    onEnter:function(){
        this._super();
        var layer = new PuzzleLayer3();
        this.addChild(layer);
    }
});